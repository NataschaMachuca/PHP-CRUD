    ...
    <table id='example' class='table table-striped' style='width:100%'>
        ...
        while ($data = $datos ->fetch(PDO::FETCH_ASSOC)){
          print "<tr>
              ...
              <td>
                 <input type='submit' id='Actualizar' name='Actualizar' value='Actualizar' onclick='validar(". $data['ID'].")'/>     
            </td>
...
function validar(id){
    var dato = confirm("Seguro que desea actualizar el coordinador.? " + id);
  if(!dato){
...
function borrar(id){
    var dato = confirm("Seguro que desea borrar el coordinador.? " + id);
  if(!dato){
    window.location.href="../views/coordinador.php";
  }else{
